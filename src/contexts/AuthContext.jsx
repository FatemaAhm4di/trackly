...
export function AuthProvider({ children }) {
  ...
  const saveNotifications = (userId, notifs) => {
    if (userId) localStorage.setItem(`notifications_${userId}`, JSON.stringify(notifs))
  }
  ...
  const addNotification = (type, data) => {
    ...
    const newNotification = { id: Date.now(), type, title: titles[type] || 'New Notification', message: messages[type] || 'You have a new notification', data: data || {}, read: false, createdAt: new Date().toISOString() }
  ...
  const login = async (email, password) => {
    ...
    try {
      ...
      if (!foundUser) return { success: false, error: 'auth/user-not-found' }
      const loggedUser = { id: foundUser.id, email: foundUser.email, name: foundUser.name || foundUser.email.split('@')[0], avatar: foundUser.avatar || '' }
      setUser(loggedUser)
  ...
  const register = async (email, password, name) => {
    ...
    try {
      const users = JSON.parse(localStorage.getItem('trackly_users') || '[]')
      if (users.find(u => u.email === email)) return { success: false, error: 'auth/email-already-in-use' }
      const newUser = { id: Date.now().toString(), email, password, name, avatar: '' }
      users.push(newUser)
      localStorage.setItem('trackly_users', JSON.stringify(users))
      const loggedUser = { id: newUser.id, email: newUser.email, name: newUser.name, avatar: '' }
      setUser(loggedUser)
      localStorage.setItem('trackly_user', JSON.stringify(loggedUser))
  ...
  const loginWithGoogle = async () => {
    setLoading(true)
    try {
      const mockUser = { id: 'google_' + Date.now(), email: `user_${Date.now()}@gmail.com`, name: 'Google User', avatar: '' }
      setUser(mockUser)
  ...
  const loginWithGithub = async () => {
    setLoading(true)
    try {
      const mockUser = { id: 'github_' + Date.now(), email: `user_${Date.now()}@github.com`, name: 'GitHub User', avatar: '' }
      setUser(mockUser)
      localStorage.setItem('trackly_user', JSON.stringify(mockUser))
      return { success: true }
    } catch (error) {
  ...
  const updateUserProfile = async ({ name, avatar }) => {
    ...
    try {
      ...
      if (idx !== -1) { users[idx].name = name; users[idx].avatar = avatar; localStorage.setItem('trackly_users', JSON.stringify(users)) }
      return { success: true }
    } catch (error) {
      return { success: false, error: error.message }
    }
  ...
  const value = { user, loading, login, register, loginWithGoogle, loginWithGithub, logout, updateUserProfile, notifications, unreadCount, addNotification, markAsRead, markAllAsRead, deleteNotification, clearAllNotifications }
